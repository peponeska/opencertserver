...
public sealed partial class Tpm2 : IDisposable
{
    ...
    /// </summary>
    private readonly List<SessionBase> TempSessions = [];
    ...
    /// </summary>
    private readonly List<TpmHandle> TempNames = [];
        ...
        int numOutHandlesNotUsed)
    {
        ...
        if (TheAlternateActionCallback != null)
        {
            ...
            var alternate = TheAlternateActionCallback(ordinal,
                inParms,
                outParms.GetType(),
                out var outParmsAlt,
        ...
        var commandInfo = CommandInfoFromCommandCode(ordinal);
        byte[] parms;
        TpmHandle[] inHandles;
        ...
        do
            try
            {
                ...
                var inSessions = CreateRequestSessions(parms, inHandles);
                ...
                var command = CommandProcessor.CreateCommand(ordinal, inHandles, inSessions, parms);
                ...
                for (; ; )
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            var respCode = CommandProcessor.GetResponseCode(response);
                    ...
                    var delay = (int)GetProperty(this, Pt.NvWriteRecovery) + 100;
#if WINDOWS_UWP
                ...
                if (repeat && resultCode == TpmRc.Success)
                {
                    ...
                    for (var i = 0; i < actualHandles.Length; ++i)
                    {
                        for (var j = 0; j < inHandles.Length; ++j)
                        {
        ...
        var commandSucceeded = ProcessError(responseTag, responseParamSize, resultCode, inParms);
        try
        {
            if (commandSucceeded)
            {
    ...
    public static string GetErrorString(Type inParmsType, uint resultCode, out TpmRc theMaskedError)
    {
        ...
        var formatOneErrorType = ((resultCodeValue & 0x80) != 0);
        var resultCodeMask = formatOneErrorType ? 0xBFU : 0x97FU;

        // Extract the actual error code
        var maskedErrorVal = resultCode & resultCodeMask;
        var maskedError = (TpmRc)maskedErrorVal;
        theMaskedError = maskedError;

        var errorEntity = "Unknown";
        uint errorEntityIndex = 0;
        var errorParmName = "Unknown";
        ...
        TpmStructureBase inParms)
    {
        ...
        var formatOneErrorType = TpmErrorHelpers.IsFmt1(resultCode);
    ...
    public static TpmRc GetBaseErrorCode(TpmRc resultCode)
    {
        ...
        // Extract the actual error code
        var maskedErrorVal = (uint)resultCode & resultCodeMask;
        var maskedError = (TpmRc)maskedErrorVal;
        return maskedError;
    }
    ...
    private static string GetParmName(Type inParmType, uint parmNumber)
    {
        ...
        var cmdName = inParmType.Name.Substring(4, inParmType.Name.Length - 11);
        var mi = typeof(Tpm2).GetMethod(cmdName);
            ...
            if (i < commandInfo.AuthHandleCountIn)
            {
                ...
                switch (authHandle.handle)
                {
                    ...
                    default:
                        ...
                        {
                            var pcrId = (int)authHandle.GetOffset();
    ...
    private SessionIn[] CreateRequestSessions(byte[] parms, TpmHandle[] inHandles)
    {
        ...
        foreach (var s in Sessions)
        {
            ...
            else if (s is AuthSession)
            {
                ...
                var parmHash = GetCommandHash(sess.AuthHash, parms, inHandles);
                var hmac = sess.GetAuthHmac(parmHash,
                    Direction.Command,
        ...
        byte[] outParmsNoHandles)
    {
        ...
        foreach (var s in Sessions)
        {
            ...
            else if (s is AuthSession)
            {
                ...
                var parmsHash = GetExpectedResponseHash(sess.AuthHash, outParmsNoHandles, commandCode, responseCode);
                var expectedHmac = sess.GetAuthHmac(parmsHash, Direction.Response);
    ...
    private void CheckParamEncSessCandidate(AuthSession candidate, SessionAttr directionFlag)
    {
        ...
        var decrypt = directionFlag == SessionAttr.Decrypt;
    ...
    private byte[] DoParmEncryption(byte[] parms, CommandInfo commandInfo, int offset, Direction inOrOut)
    {
        ...
        var countSize = commandInfo.TheParmCryptInfo.HasFlag(encFlag2) ? 2 : 4;
        ...
        // prepended buffer
        var lenX = Globs.CopyData(parms, offset, countSize);
        var len = Globs.NetToHostVar(lenX);
        var firstParm = Globs.CopyData(parms, offset + countSize, (int)len);
        var encFirstParm = encSess.ParmEncrypt(firstParm, inOrOut);
    ...
    private byte[] GetCommandHash(TpmAlgId hashAlg, byte[] commandParms, TpmHandle[] handles)
    {
        ...
        var parmsHash = CryptoLib.HashData(hashAlg, temp.GetBytes());
    ...
    public static CommandInfo CommandInfoFromCommandCode(TpmCc commandCode)
    {
        ...
        foreach (var theInfo in CommandInformation.Info)
        {
...
public class CommandProcessor
{
        ...
        out byte[] commandParms)
    {
        ...
        var commandInfo = Tpm2.CommandInfoFromCommandCode(header.CommandCode);
        ...
        if (header.Tag == TpmSt.Sessions)
        {
            ...
            var sessionEnd = m.GetGetPos() + sessionLength;
        ...
        byte[] parmsWithoutHandles)
    {
        ...
        var tag = sessions.Length == 0 ? TpmSt.NoSessions : TpmSt.Sessions;
        m.Put(tag, "tag");
        m.PushLength(4);
        ...
        out byte[] responseParmsWithHandles)
    {
        ...
        if (responseCode != TpmRc.Success)
        {
            handles = [];
            sessions = [];
            responseParmsNoHandles = [];
            responseParmsWithHandles = [];
            return;
        }
        ...
        var startOfParms = m.GetGetPos();
        var parmsLength = parmsEnd - startOfParms;
    ...
    public static string ParseCommand(byte[] buf)
    {
        ...
        var ok = CrackCommand(buf, out commandHeader, out inHandles, out inSessions, out commandParmsNoHandles);
        if (!ok)
        {
        ...
        var commandParmsWithHandles = Globs.Concatenate([m2.GetBytes(), commandParmsNoHandles]);
    ...
    public static string ParseResponse(string commandCode, byte[] buf)
    {
        ...
        var responseIsEncrypted = false;
        foreach (var s in outSessions)
        {
...
internal class ReentrancyGuard : IDisposable
{
    ...
    internal ReentrancyGuard(ReentrancyGuardContext theInstance)
    {
        ...
        var newCount = Interlocked.Increment(ref Instance.ThreadCount);
        if (newCount != 1)
        {
